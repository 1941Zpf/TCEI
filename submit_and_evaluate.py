...
def submit_and_evaluate(config: dict):
    ...
    metrics = submit_and_evaluate_one_model(
        ...
        pos_enabled=config['POS_ENABLED'],
        pos_shot_capacity=config['POS_SHOT_CAPACITY'],
        pos_alpha=config['POS_ALPHA'],
        pos_beta=config['POS_BETA'],
        neg_enabled=config['NEG_ENABLED'],
        neg_shot_capacity=config['NEG_SHOT_CAPACITY'],
        neg_alpha=config['NEG_ALPHA'],
        neg_beta=config['NEG_BETA'],
        neg_entropy_threshold_lower=config['NEG_ENTROPY_THRESHOLD_LOWER'],
        neg_entropy_threshold_upper=config['NEG_ENTROPY_THRESHOLD_UPPER'],
        neg_mask_threshold_lower=config['NEG_MASK_THRESHOLD_LOWER'],
        neg_mask_threshold_upper=config['NEG_MASK_THRESHOLD_UPPER'],
        pos_temperature=config['POS_TEMPERATURE'],
        neg_temperature=config['NEG_TEMPERATURE'],
        w_pos_enabled=config['W_POS_ENABLED'],
        w_pos_shot_capacity=config['W_POS_SHOT_CAPACITY'],
        w_pos_alpha=config['W_POS_ALPHA'],
        w_pos_beta=config['W_POS_BETA'],
        w_neg_enabled=config['W_NEG_ENABLED'],
        w_neg_shot_capacity=config['W_NEG_SHOT_CAPACITY'],
        w_neg_alpha=config['W_NEG_ALPHA'],
        w_neg_beta=config['W_NEG_BETA'],
        w_neg_entropy_threshold_lower=config['W_NEG_ENTROPY_THRESHOLD_LOWER'],
        w_neg_entropy_threshold_upper=config['W_NEG_ENTROPY_THRESHOLD_UPPER'],
        w_neg_mask_threshold_lower=config['W_NEG_MASK_THRESHOLD_LOWER'],
        w_neg_mask_threshold_upper=config['W_NEG_MASK_THRESHOLD_UPPER'],
        w_pos_temperature=config['W_POS_TEMPERATURE'],
        w_neg_temperature=config['W_NEG_TEMPERATURE'],
...
def submit_and_evaluate_one_model(
    ...
    for sequence_name in inference_dataset.sequence_infos.keys():
        ...
        runtime_tracker = RuntimeTracker(
            ...
            neg_alpha=neg_alpha,
            neg_beta=neg_beta,
            neg_entropy_threshold_lower=neg_entropy_threshold_lower,
            neg_entropy_threshold_upper=neg_entropy_threshold_upper,
            neg_mask_threshold_lower=neg_mask_threshold_lower,
            neg_mask_threshold_upper=neg_mask_threshold_upper,
            pos_temperature=pos_temperature,
            neg_temperature=neg_temperature,
            w_pos_enabled=w_pos_enabled,
            w_pos_shot_capacity=w_pos_shot_capacity,
            w_pos_alpha=w_pos_alpha,
            w_pos_beta=w_pos_beta,
            w_neg_enabled=w_neg_enabled,
            w_neg_shot_capacity=w_neg_shot_capacity,
            w_neg_alpha=w_neg_alpha,
            w_neg_beta=w_neg_beta,
            w_neg_entropy_threshold_lower=w_neg_entropy_threshold_lower,
            w_neg_entropy_threshold_upper=w_neg_entropy_threshold_upper,
            w_neg_mask_threshold_lower=w_neg_mask_threshold_lower,
            w_neg_mask_threshold_upper=w_neg_mask_threshold_upper,
            w_pos_temperature=w_pos_temperature,
            w_neg_temperature=w_neg_temperature,
        ...
        sequence_results, sequence_fps = get_results_of_one_sequence(
            ...
            logger=logger,
            w_pos_cache=w_pos_cache,
            w_neg_cache=w_neg_cache,
        )
...
def get_results_of_one_sequence(
    ...
    for t, (image, image_path) in enumerate(sequence_loader):
        ...
        runtime_tracker.update(image=image, pos_cache=pos_cache, neg_cache=neg_cache, w_pos_cache=w_pos_cache, w_neg_cache=w_neg_cache)
